...
contract MARIO is Context, IERC20, SafeToken, LockToken {
    ...
    uint256 private _tTotal = 19868801 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "MARIO WORLD";
    string private _symbol = "SHROOMS";
    ...
    uint256 private minimumTokensBeforeSwap = _maxTxAmount / 10;
    ...
    function _transfer(
        ...
        require(!blackList[from], "Black list address!");
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        marketingAddress.transfer(
            transferredBalance.div(_burnFee).mul(marketingDivisor)
        );
    }

    function buyBackTokens() private lockTheSwap {
        if (address(this).balance > buyBackUpperLimit) {
            swapETHForTokens(buyBackUpperLimit.div(100));
        }
    }
    ...
    function excludeFromFee(address account, bool isExclude) public onlyOwner {
        _isExcludedFromFee[account] = isExclude;
    }
    ...
    function setMinSwapToken(uint256 _minimumTokensBeforeSwap)
        external
        onlyOwner
    {
    ...
    function enableSwap(bool _enabled) public onlyOwner {
    ...
    function setBlackList(address adr, bool black) external onlyOwner {
        blackList[adr] = black;
    }
